...
export default function GradesPage() {
    ...
    try {
      ...
      studentDocs.forEach((docSnapshot, index) => {
        ...
        if (docSnapshot?.exists()) {
          ...
          studentNames[studentId] = data.name || `Student ${studentId.slice(-1)}`;
        } else {
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (data.courseId === selectedCourseId) {
            ...
            const activities: GradeSheetActivity[] = (data.activities || []).map((act: any, index: number) => ({
            ...
            rawStudents.forEach((student: any) => {
              ...
              if (student.grades) {
                Object.entries(student.grades).forEach(([key, gradeData]: [string, any]) => {
                  grades[key] = {
              ...
              students.push({
                ...
                name: student.name || `Student ${student.studentId?.slice(-1) || 'Unknown'}`,
                grades,
        ...
        if (isTeacher && studentIdsToFetch.size > 0) {
          const studentNames = await fetchStudentNames(Array.from(studentIdsToFetch));
  ...
  const weightValidation = gradeSheets.length > 0 ? validateWeightPercentages() : { isValid: false, total: 0 };

  // Datos del estudiante
  const studentData = useMemo(() => {
    if (!user || isTeacher || !selectedCourseId || gradeSheets.length === 0) return null;
    ...
    const totalWeightPercentage = publishedSheets.reduce((total, sheet) => 
      total + (Number(sheet.weightPercentage) || 0), 0
    );
  ...
  const studentsWithGrades = useMemo((): StudentWithGrades[] => {
    ...
    return selectedCourse.enrolledStudents
      .map(studentId => {
        let studentName = courseStudents[studentId] || 
          gradeSheets.reduce((foundName: string, sheet) => {
        ...
        studentSheets.forEach(sheet => {
          const studentData = sheet.students.find(s => s.studentId === studentId);
          if (!studentData) return;
...
const getCourseCode = (): string => {
  ...
  const filteredStudents = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        case 'name':
          ...
          break;
        case 'grade':
          comparison = a.systemProgress.currentGrade - b.systemProgress.currentGrade;
          break;
        case 'status':
          const statusOrder = { 'passing': 0, 'at-risk': 1, 'failing': 2 };
          comparison = statusOrder[a.systemProgress.status] - statusOrder[b.systemProgress.status];
          break;
      }
    ...
    return filtered;
  }, [studentsWithGrades, filterByStatus, sortBy, sortOrder]);

  // Estadísticas del profesor
  const teacherStats = useMemo(() => {
    const passing = filteredStudents.filter(s => s.systemProgress.status === 'passing').length;
    const atRisk = filteredStudents.filter(s => s.systemProgress.status === 'at-risk').length;
    const failing = filteredStudents.filter(s => s.systemProgress.status === 'failing').length;
    ...
    return {
      ...
      averageGrade: totalStudents > 0 
        ? filteredStudents.reduce((sum, s) => sum + s.systemProgress.currentGrade, 0) / totalStudents
        : 0,
      publishedSheets: gradeSheets.filter(s => s.isPublished).length,
      totalActivities: gradeSheets.reduce((total, sheet) => total + sheet.activities.length, 0),
  ...
  const exportToExcel = () => {
    ...
    const exportData = filteredStudents.map(student => ({
      ...
      'Status': student.systemProgress.status === 'passing' ? 'Passing' :
                student.systemProgress.status === 'at-risk' ? 'At Risk' : 'Failing',
    ...
    XLSX.writeFile(wb, `Grades_${selectedCourse.code}_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const getStatusBadgeClass = (status: string) => {
    switch (status) {
      case 'passing': return 'bg-gradient-to-r from-green-100 to-emerald-100 text-green-700 border border-green-200';
      case 'at-risk': return 'bg-gradient-to-r from-amber-100 to-orange-100 text-amber-700 border border-amber-200';
      case 'failing': return 'bg-gradient-to-r from-red-100 to-pink-100 text-red-700 border border-red-200';
        ...
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center space-y-2">
            ...
            <div className="space-y-2">
              <p className="text-lg font-semibold text-gray-900">Loading grades</p>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex-1">
              <p className="text-sm font-medium text-gray-600 mb-2">Select Course</p>
              <div className="relative">
                <select
                  value={selectedCourseId}
                  onChange={(e) => handleCourseChange(e.target.value)}
                  className="w-full px-4 py-3.5 pr-10 bg-gray-50 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all appearance-none cursor-pointer hover:bg-gray-100 text-sm font-medium"
                >
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {studentData.status === 'passing' && studentData.currentGrade >= 4.0 && (
              ...
              <div className="grid grid-cols-4 sm:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
                <div className="bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-100 rounded-xl p-2 md:p-5">
                  <p className="text-xs font-semibold text-blue-600  tracking-wide mb-2">Current Average</p>
                  <div className="flex items-end gap-2">
                    <p className="text-xl font-bold text-gray-900 text-center md:text-left">
                      {formatGrade(studentData.currentGrade)}
                    </p>
                    <span className="text-sm text-gray-500 mb-1 hidden md:block">/5.0</span>
                  </div>
                  {studentData.currentGrade >= 4.0 && (
                    <div className="flex items-center gap-1 mt-2">
                      <Star className="h-3 w-3 text-amber-500" />
                      <span className="text-xs text-amber-600 font-medium">Top Performance</span>
                    </div>
                  )}
                </div>
                
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-100 rounded-xl p-2 md:p-5">
                  <p className="text-xs font-semibold text-green-600  tracking-wide mb-2">Evaluated</p>
                  <p className="text-xl font-bold text-gray-900 text-center md:text-left">
                ...
                <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-100 rounded-xl p-2 md:p-5">
                  <p className="text-xs font-semibold text-amber-600  tracking-wide mb-2">Remaining</p>
                  <p className="text-xl font-bold text-gray-900 text-center md:text-left">
                ...
                <div className="bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-100 rounded-xl p-2 md:p-5">
                  <p className="text-xs font-semibold text-purple-600  tracking-wide mb-2">To Pass</p>
                  <p className="text-xl font-bold text-gray-900 text-center md:text-left">
                    {studentData.minGradeToPass > 0 ? studentData.minGradeToPass.toFixed(1) : '3.0'}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-900">Weight Distribution</h4>
                      <p className="text-sm text-gray-500">How each period contributes to your final grade</p>
                    </div>
                  </div>
                  {studentData.totalWeightPercentage !== 100 && (
                    <Badge className="bg-gradient-to-r from-amber-100 to-orange-100 text-amber-700 border border-amber-200">
                      {studentData.totalWeightPercentage < 100 ? 'Incomplete' : 'Over 100%'}
                    </Badge>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  {gradeSheets
                    ...
                    .map(sheet => (
                      <div key={sheet.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-xl hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-cyan-50/50 transition-all duration-300">
                        <div className="flex items-center gap-3">
                  ...
                  <div className="pt-4 mt-4 border-t border-gray-200">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">Sum of all periods</p>
                      </div>
                      <span className={`text-xl font-bold ${studentData.totalWeightPercentage === 100 ? 
                        'text-green-600' : studentData.totalWeightPercentage > 100 ? 'text-amber-600' : 'text-blue-600'
                      }`}>
                          ...
                          {studentData.totalWeightPercentage > 100 ? (
                            <>
                              ...
                              <p className="text-sm text-amber-700">
                                Exceeds 100% by {(studentData.totalWeightPercentage - 100).toFixed(1)}%
                              </p>
                            </>
                          ) : (
                            <>
                              ...
                              <p className="text-sm text-blue-700">
                                {(100 - studentData.totalWeightPercentage).toFixed(1)}% remaining to complete
                              </p>
                            </>
                          )}
                        </div>
              ...
              <div className="px-6 py-5 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-gray-900">Grades by Period</h3>
                  ...
                  <Badge className="bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-700 border border-blue-200 flex items-center gap-1 max-w-[25%]">
                    ...
                    {selectedCourse.semester ? `${selectedCourse.semester} Term` : 'Current'}
                  </Badge>
                </div>
              </div>
                ...
                {gradeSheets.length === 0 ? (
                  <div className="text-center py-10 px-4">
                    ...
                    <p className="text-sm text-gray-600 max-w-md mx-auto">
                      Your grades will appear here once the teacher publishes evaluations for this course.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {gradeSheets.map(sheet => {
                      const updatedAt = firestoreTimestampToDate(sheet.updatedAt);
                        ...
                        >
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500">
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  <span>{updatedAt.toLocaleDateString('en-US', { 
                                    month: 'short', 
                                    day: 'numeric',
                                    year: 'numeric'
                                  })}</span>
                                ...
                                {(sheet.weightPercentage || 0) > 0 && (
                                  <>
                                    ...
                                    <div className="flex items-center gap-1 hidden sm:block">
                                      <span className="font-semibold text-blue-600">{sheet.weightPercentage || 0}%</span>
                                    </div>
                                  </>
                                )}
                              </div>
                          ...
                          <div className="space-y-2">
                            {sheet.activities.map(activity => {
                              const grade = studentSheetData?.grades[activity.id];
                              const gradeValue = grade?.value || 0;
                              const isExcellent = gradeValue >= 4.0;
                              const isGood = gradeValue >= 3.0 && gradeValue < 4.0;
                                ...
                                >
                                  <div className="flex-1 min-w-0">
                                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-1">
                                      <p className="font-semibold text-gray-900 text-xs sm:text-sm">{activity.name}</p>
                                      <Badge className={`text-xs max-w-[50%] ${
                                        activity.type === 'exam' ? 'bg-gradient-to-r from-red-100 to-pink-100 text-red-700 border border-red-200' :
                                        activity.type === 'quiz' ? 'bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-700 border border-blue-200' :
                                        activity.type === 'homework' ? 'bg-gradient-to-r from-green-100 to-emerald-100 text-green-700 border border-green-200' :
                                        activity.type === 'project' ? 'bg-gradient-to-r from-purple-100 to-pink-100 text-purple-700 border border-purple-200' :
                                        ...
                                        {activity.type === 'exam' ? 'Exam' :
                                         ...
                                         activity.type === 'participation' ? 'Participation' : activity.type}
                                      </Badge>
                                    </div>
                                  </div>
                                      ...
                                      <div className="flex flex-col items-end">
                                        <div className="flex items-center gap-2">
                                          <span className={cn(
                                            ...
                                            gradeValue >= 2.0 ? 'text-gray-600' : 'text-gray-700'
                                          )}>
                          ...
                          {(() => {
                            ...
                            return periodCount > 0 ? (
                              <div className="mt-6 pt-4 border-t border-gray-200">
                                <div className="flex items-center justify-between">
                                  <span className="font-bold text-gray-900">Period average:</span>
                                  <div className="flex items-center gap-3">
                                    <span className={cn(
                                      'text-2xl font-bold pr-6',
                                      periodAverage >= 3.0 ? 'text-green-600' :
                                      periodAverage >= 2.0 ? 'text-amber-600' : 'text-red-600'
                                    )}>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-xs font-semibold text-blue-600  tracking-wide">Students</p>
                    </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-xs font-semibold text-purple-600  tracking-wide">Average Grade</p>
                    </div>
                    <p className="text-2xl font-bold text-gray-900">{teacherStats.averageGrade.toFixed(1)}</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-xs font-semibold text-green-600  tracking-wide">Total Activities</p>
                    </div>
                    <p className="text-2xl font-bold text-gray-900">{teacherStats.totalActivities}</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-xs font-semibold text-amber-600  tracking-wide">Published Sheets</p>
                    </div>
                    <p className="text-2xl font-bold text-gray-900">{teacherStats.publishedSheets}</p>
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-2xl font-bold text-gray-900">{teacherStats.passing}</span>
                </div>
                <div className="w-full h-3 bg-gradient-to-r from-gray-100 to-gray-200 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-green-500 to-emerald-500 transition-all duration-500"
                    style={{ width: `${teacherStats.totalStudents > 0 ? (teacherStats.passing / teacherStats.totalStudents) * 100 : 0}%` }}
                  />
                </div>
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-2xl font-bold text-gray-900">{teacherStats.atRisk}</span>
                </div>
                <div className="w-full h-3 bg-gradient-to-r from-gray-100 to-gray-200 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-amber-500 to-orange-500 transition-all duration-500"
                    style={{ width: `${teacherStats.totalStudents > 0 ? (teacherStats.atRisk / teacherStats.totalStudents) * 100 : 0}%` }}
                  />
                </div>
                <p className="text-sm text-gray-500 mt-3">
                  {teacherStats.totalStudents > 0 
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-2xl font-bold text-gray-900">{teacherStats.failing}</span>
                </div>
                <div className="w-full h-3 bg-gradient-to-r from-gray-100 to-gray-200 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-red-500 to-pink-500 transition-all duration-500"
                    style={{ width: `${teacherStats.totalStudents > 0 ? (teacherStats.failing / teacherStats.totalStudents) * 100 : 0}%` }}
                  />
                </div>
                <p className="text-sm text-gray-500 mt-3">
                  {teacherStats.totalStudents > 0 
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-900">Weight Distribution</h4>
                      <p className="text-sm text-gray-500">How each period contributes to final grades</p>
                    </div>
                  </div>
                  <Badge className={weightValidation.isValid ? 
                      ...
                      <div key={sheet.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-xl hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-cyan-50/50 transition-all duration-300">
                        <div className="flex items-center gap-3">
                          <div className={`h-10 w-10 rounded-lg flex items-center justify-center ${
                            ...
                            <FileSpreadsheet className={`h-5 w-5 ${sheet.isPublished ? 'text-green-500' : 'text-amber-500'}`} />
                          </div>
                          <div>
                            <p className="font-semibold text-gray-900">{sheet.title}</p>
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`text-lg font-bold ${(sheet.weightPercentage || 0) > 0 ? 'text-blue-600' : 'text-gray-500'}`}>
                            {sheet.weightPercentage || 0}%
                          </span>
<Link 
  ...
  Edit
                  ...
                  <div className="pt-4 mt-4 border-t border-gray-200">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-xl font-bold ${weightValidation.isValid ? 
                        'text-green-600' : weightValidation.total > 100 ? 'text-amber-600' : 'text-blue-600'
                      }`}>
                        {weightValidation.total}%
                      </span>
                    </div>
                        ...
                        <div className="flex items-start gap-2">
                          {weightValidation.total > 100 ? (
                            <>
                              <AlertTriangle className="h-4 w-4 text-amber-600 mt-0.5 flex-shrink-0" />
                              <p className="text-sm text-amber-700">
                                Exceeds 100% by {(weightValidation.total - 100).toFixed(1)}%
                              </p>
                            </>
                          ) : (
                            <>
                              <Target className="h-4 w-4 text-blue-600 mt-0.5 flex-shrink-0" />
                              <p className="text-sm text-blue-700">
                                {(100 - weightValidation.total).toFixed(1)}% below target (should be 100%)
                              </p>
                            </>
                          )}
                        </div>
                ...
                <div className="px-6 py-5 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <FileSpreadsheet className="h-5 w-5 text-blue-500" />
                        <h3 className="text-xl font-bold text-gray-900">Student Weighted Averages</h3>
                      </div>
                      <p className="text-sm text-gray-500">
                        {selectedCourse.code} - {selectedCourse.name}
                        {filterByStatus !== 'all' && ` • Filtered by: ${filterByStatus === 'passing' ? 'Passing' : filterByStatus === 'at-risk' ? 'At Risk' : 'Failing'}`}
                      </p>
                    </div>
                    <div className="text-sm font-medium text-gray-600">
                      {filteredStudents.length} of {selectedCourse.enrolledStudents.length} students
                    </div>
                  </div>
                </div>
                        ...
                        <tr key={student.id} className={cn(
                          ...
                          <td className="text-center px-4 py-4">
                            <div className="flex flex-col items-center">
                              <span className={cn(
                                "text-xl font-bold",
                                student.systemProgress.currentGrade >= 3.0 ? "text-green-600" :
                                student.systemProgress.currentGrade >= 2.0 ? "text-amber-600" : "text-red-600"
                              )}>
                                {formatGrade(student.systemProgress.currentGrade)}
                              </span>
                          ...
                          <td className="text-center px-4 py-4">
                            <Badge
                              className={getStatusBadgeClass(student.systemProgress.status)}
                            >
                              {student.systemProgress.status === 'passing' && student.systemProgress.currentGrade >= 4.0 && (
                          ...
                          <td className="text-center px-4 py-4">
                            <div className="flex flex-col items-center">
                              <span className="font-semibold text-gray-900">{student.completedAssessments}/{student.totalAssessments}</span>
                              ...
                              {student.totalAssessments > 0 && (
                                <div className="w-20 h-1.5 bg-gray-200 rounded-full overflow-hidden mt-1">
                                  <div 
                                    ...
                                    style={{ width: `${(student.completedAssessments / student.totalAssessments) * 100}%` }}
                                  />
                                </div>
                              )}
                            </div>
            ...
            {gradeSheets.length === 0 && (
              <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center shadow-sm">
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-2">No grade sheets</h3>
                <p className="text-gray-600 max-w-md mx-auto mb-6">
                  No grade sheets have been created for this course yet. 
                  Start by creating your first grade sheet to manage grades.
                </p>
            ...
            {selectedCourse.enrolledStudents.length > 0 && filteredStudents.length === 0 && gradeSheets.length > 0 && (
              <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center shadow-sm">
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-2">No students match the filters</h3>
                <p className="text-gray-600 max-w-md mx-auto">
            ...
            {selectedCourse.enrolledStudents.length === 0 && (
              <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center shadow-sm">
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-2">No enrolled students</h3>
                <p className="text-gray-600 max-w-md mx-auto">
                  No students are enrolled in this course. Add students to start managing grades.
                </p>
              </div>
            )}
          </div>
