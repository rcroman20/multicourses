...
export default function ContactPage() {
  ...
  const priorityResponseHours = Math.max(1, Number(settings.defaultResponseHoursPriority) || 24);
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-slate-100 px-4 py-5 sm:px-6 lg:px-8">
      <SeoHead
        ...
        description={`Contact the ${platformName} team for teacher requests, payment questions, plan recommendations, technical support, and institution onboarding.`}
        canonicalPath="/contact"
        keywords={`contact ${platformName}, teacher approval support, education platform support, institution onboarding`}
        structuredData={{
      ...
      <div className="relative mx-auto w-full max-w-[1200px]">
        <PublicTopNav activeKey="contact" />
        <div className="rounded-2xl border border-slate-200/60 bg-white p-4 shadow-[0_12px_35px_-24px_rgba(15,23,42,0.45)] lg:p-6">

        {/* Hero Section */}
        <section className="mb-8 rounded-2xl border border-slate-200/60 bg-gradient-to-br from-white via-slate-50 to-sky-50 p-5 shadow-sm lg:p-6">
          <div className="max-w-2xl">
            ...
            <h1 className="mb-3 text-xl font-extrabold tracking-tight text-slate-900 sm:text-2xl">
              Talk to the {platformName} team
            </h1>
            <p className="text-sm leading-relaxed text-slate-600">
              Send your request directly to our admin panel for quick follow-up. 
              We're here to help with any questions about our platform, with a typical response target of {starterResponseHours} hours.
            </p>
          </div>
        </section>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            <div className="rounded-2xl border border-slate-200/60 bg-white p-5 shadow-sm lg:p-6">
              <div className="mb-5">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full resize-none rounded-lg border border-slate-300/60 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-sky-400 focus:ring-2 focus:ring-sky-100"
                  />
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-slate-200/60 bg-white p-4 shadow-sm">
              ...
              <h3 className="mb-2 text-sm font-semibold text-slate-900">Response times</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                  <span className="text-sm text-slate-600">Starter plan</span>
                  <span className="text-sm font-semibold text-slate-900">Up to {starterResponseHours}h</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-600">Growth / Scale</span>
                  <span className="text-sm font-semibold text-slate-900">Up to {priorityResponseHours}h</span>
                </div>
              </div>
            </div>

            {(supportPhone || normalizedWhatsApp) && (
              <div className="rounded-2xl border border-slate-200/60 bg-white p-4 shadow-sm">
                <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100">
                  <Phone className="h-4 w-4 text-slate-700" />
                </div>
                <h3 className="mb-2 text-sm font-semibold text-slate-900">Direct lines</h3>
                <div className="space-y-2">
                  {supportPhone ? (
                    <a
                      href={`tel:${supportPhone}`}
                      className="flex items-center justify-between rounded-lg bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-100"
                    >
                  ...
                  {normalizedWhatsApp ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-between rounded-lg bg-emerald-50 px-4 py-2.5 text-sm font-medium text-emerald-700 transition hover:bg-emerald-100"
                    >
                      <span className="inline-flex items-center gap-2">
            ...
            {/* Quick Actions Card */}
            <div className="rounded-2xl border border-slate-200/60 bg-gradient-to-br from-emerald-50 to-white p-4 shadow-sm">
              <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-100">
                <ShieldCheck className="h-4 w-4 text-emerald-700" />
              </div>
              <h3 className="mb-2 text-sm font-semibold text-slate-900">Need immediate help?</h3>
        ...
        {/* Help Topics Section */}
        <section className="mt-8 rounded-2xl border border-slate-200/60 bg-white p-5 shadow-sm lg:p-6">
          <div className="mb-5 text-center">
            <MessageSquare className="mx-auto mb-3 h-6 w-6 text-indigo-600" />
            <h2 className="text-xl font-extrabold text-slate-900 sm:text-2xl">What we can help you with</h2>
            <p className="mt-1 text-sm text-slate-600">
              Common topics our support team handles every day. Priority cases are targeted within {priorityResponseHours} hours.
            </p>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {helpTopics.map((topic, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex items-start gap-3 rounded-lg border border-slate-100 bg-slate-50 p-4 transition hover:border-slate-200/60 hover:bg-white"
                >
        ...
        {/* Trust Indicators */}
        <section className="mt-6 rounded-2xl border border-slate-200/60 bg-gradient-to-br from-sky-50 to-indigo-50 p-5">
        ...
        <PublicFooter summary={`Reach the ${platformName} team for onboarding, support, payment guidance, and operational questions.`} />
        </div>
      </div>
    </div>
  );
