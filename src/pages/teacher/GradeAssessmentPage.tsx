  ...
  <div className="space-y-2">
    ...
    <div className="modern-card">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-2">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-gray-100 border border-gray-300 rounded-xl p-4 animate-pulse">
            <div className="flex items-center gap-3">
        ...
        {[...Array(4)].map((_, i) => (
          <div key={i} className="modern-card">
            <div className="text-center">
              <div className="h-3 w-24 bg-gray-200 rounded mx-auto mb-2 animate-pulse"></div>
              <div className="h-8 w-16 bg-gray-200 rounded mx-auto animate-pulse"></div>
              <div className="h-2 w-20 bg-gray-200 rounded mx-auto mt-2 animate-pulse"></div>
            </div>
          </div>
        ))}
      </div>
...
export default function GradeAssessmentPage() {
  ...
  const loadForumSubmissionsDirectly = async (assessmentId: string, currentCourseId: string): Promise<any[]> => {
    try {
      const q = query(
        collection(db, 'assessmentForumComments'),
        where('assessmentId', '==', assessmentId),
      ...
      querySnapshot.forEach((commentDoc) => {
        ...
        const studentId = String(data.userId || '').trim();
        if (!studentId) return;

        let createdAt: Date | null = null;
        const rawCreatedAt = data.createdAt;
        if (rawCreatedAt?.toDate) createdAt = rawCreatedAt.toDate();
        else if (rawCreatedAt?.seconds) createdAt = new Date(rawCreatedAt.seconds * 1000);
        else if (typeof rawCreatedAt === 'string' || typeof rawCreatedAt === 'number') createdAt = new Date(rawCreatedAt);
        ...
        const prev = commentsByStudent.get(studentId) || [];
        prev.push(item);
        commentsByStudent.set(studentId, prev);
      });

      const forumSubmissions: any[] = [];
      commentsByStudent.forEach((comments, studentId) => {
        ...
        const latest = orderedComments[orderedComments.length - 1];
        const content = orderedComments
          .map((comment, index) => {
            const label = comment.parentCommentId ? 'Reply' : 'Comment';
            return `${index + 1}. [${label}] ${comment.content}`;
          })
          .join('\n\n');

        forumSubmissions.push({
          id: `forum_${assessmentId}_${studentId}`,
          studentId,
          assessmentId,
  ...
  const loadRestOfData = async (course: any, assessmentData: any) => {
    try {
      ...
      if (assessmentData.type === 'forum') {
        studentSubmissions = await loadForumSubmissionsDirectly(assessmentId!, course.id);
      } else if (assessmentData.assessmentType === 'delivery') {
        try {
...
const calculateStats = () => {
  ...
  if (!assessment) {
    return (
      <DashboardLayout title="Calificar Evaluación" subtitle="Evaluación no encontrada"      contentClassName="pt-0 lg:pt-1">
...
<div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-2">
  ...
  <div className="bg-white border border-gray-200 rounded-xl p-4">
    <div className="text-center">
      <div className="mx-auto mb-2 h-8 w-8 rounded-lg bg-blue-100 flex items-center justify-center">
        <Users className="h-4 w-4 text-blue-600" />
      </div>
      <p className="text-sm text-blue-700 font-bold">Total Students</p>
  ...
  <div className="bg-white border border-gray-200 rounded-xl p-4">
    <div className="text-center">
      ...
      <p className="text-sm text-blue-700 font-bold">Graded</p>
      <p className="text-xl font-bold text-gray-900">
  ...
  <div className="bg-white border border-gray-200 rounded-xl p-4">
    <div className="text-center">
      ...
      <p className="text-sm text-blue-700 font-bold">Average Score</p>
      <p className="text-xl font-bold text-gray-900">
  ...
  <div className="bg-white border border-gray-200 rounded-xl p-4">
    <div className="text-center">
      ...
      <div className="mt-2 text-xs text-gray-700">
        {isLoading ? '...' : `${stats.passingCount} passed • ${stats.failingCount} failed`}
      </div>
    </div>
  </div> 
  
  {/* Submitted (only for delivery) */}
  {assessment.assessmentType === 'delivery' ? (
    <div className="bg-white border border-gray-200 rounded-xl p-4">
      <div className="text-center">
        ...
        <p className="text-sm text-blue-700 font-bold">Submitted</p>
        <p className="text-xl font-bold text-gray-900">
          {isLoading ? '...' : stats.submittedCount}
        </p>
        <div className="mt-2 text-xs text-gray-700">
          {isLoading ? '...' : `${stats.submissionRate}% submitted`}
        </div>
      </div>
    </div>
  ) : (
    <div className="bg-white border border-gray-200 rounded-xl p-4">
      <div className="text-center">
        ...
        <p className="text-sm text-blue-700 font-bold">Weight</p>
        <p className="text-xl font-bold text-gray-900">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300 text-sm font-medium"
                  value={searchTerm}
              ...
              <button
                ...
                disabled={isSaving}
                className="px-5 py-3 rounded-xl bg-blue-600 text-white hover:shadow-lg transition-all duration-300 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            <div className="flex flex-wrap gap-3">
              <div className="relative">
                ...
                <select
                  className="pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300 appearance-none text-sm font-medium"
                  value={filterStatus}
                  onChange={(e) => setFilterStatus(e.target.value)}
                >
                  <option value="all">All students</option>
                  <option value="with_submission">With submission</option>
                  <option value="without_submission">Without submission</option>
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-sm text-gray-600">
          {isLoading ? 'Loading...' : `${filteredStudents.length} of ${students.length} students`}
        </p>
      </div>
    </div>
    <div className="flex items-center gap-4">
      <div className="text-sm text-gray-600">
        Max points: <span className="font-bold text-gray-900">{assessment.maxPoints}.0</span>
      </div>
      <div className="text-sm text-gray-600">
        Passing score: <span className="font-bold text-gray-900">{assessment.passingScore}.0</span>
      </div>
    </div>
  </div>
    ...
    <>
            ...
            {filteredStudents.map((student) => {
              ...
              if (student.hasSubmission) {
                switch (student.submissionStatus) {
                  case 'submitted': 
                    submissionStatusColor = 'bg-blue-100 text-blue-700 border border-blue-200';
                    submissionStatusText = 'Submitted';
                    submissionIcon = <CheckCircle className="h-3 w-3" />;
                    break;
                  case 'draft':
                    submissionStatusColor = 'bg-gray-100 text-gray-700 border border-gray-200';
                  ...
                  case 'graded':
                    ...
                    break;
                  default:
                    submissionStatusColor = 'bg-gray-100 text-gray-700 border border-gray-300';
                    submissionStatusText = 'Pending';
                    submissionIcon = <Clock className="h-3 w-3" />;
                }
              } else {
                submissionStatusColor = 'bg-red-100 text-red-700 border border-red-200';
              ...
              if (isGraded) {
                if (isPassing) {
                  gradeStatusColor = 'bg-blue-100 text-blue-700 border border-blue-200';
                  gradeStatusText = 'Approved';
                } else {
                  gradeStatusColor = 'bg-red-100 text-red-700 border border-red-200';
                  gradeStatusText = 'No Approved';
                }
              } else {
                gradeStatusColor = 'bg-gray-100 text-gray-700 border border-gray-300';
                gradeStatusText = 'Pending';
              }
              
              return (
                <tr key={student.studentId} className="hover:bg-blue-50 transition-all">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          <input
                            ...
                            className="w-20 px-4 py-1.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300 text-sm font-medium [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                            placeholder="3.5"
                            min={0}
                  ...
                  <td className="py-4 px-6">
                    <div className="relative">
                      <textarea
                        ...
                        rows={2}
                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300 text-sm font-medium resize-none"
                      />
                    </div>
                    {student.comment && (
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200 transition-all duration-300 font-medium text-sm border border-blue-200 disabled:opacity-50 disabled:cursor-not-allowed"
                        title="Ver entrega"
                      >
                      ...
                      <button
                        ...
                        disabled={!student.submissionContent}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200 transition-all duration-300 font-medium text-sm border border-blue-300 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {filteredStudents.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-bold text-gray-900 mb-2">No students match the selected filters</h3>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 mt-4 rounded-xl bg-gray-100 text-gray-700 hover:bg-gray-200 transition-all duration-300 font-medium border border-gray-300"
          >
            Limpiar filtros
          </button>
        </div>
