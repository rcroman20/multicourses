...
export default function AdminInstitutionDetailPage() {
  ...
  const teacherCount = institutionUsers.filter((entry) => getEffectiveRole(entry) === "docente").length;
  const studentCount = institutionUsers.filter((entry) => getEffectiveRole(entry) === "estudiante").length;
  const adminCount = institutionUsers.filter((entry) => getEffectiveRole(entry) === "admin").length;
          ...
          <section className="space-y-4">
            <section className="relative overflow-hidden rounded-2xl border border-slate-200/60 bg-gradient-to-br from-white via-slate-50 to-sky-50 p-4 shadow-sm">
              ...
              <div className="relative space-y-4">
                ...
                <div className="inline-flex items-center gap-2 rounded-full border border-slate-200/60 bg-white px-3 py-1 text-xs font-semibold text-slate-700">
                  <Building2 className="h-3.5 w-3.5 text-sky-700" />
                  {institutionLabel}
                </div>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-5">
                  <div className="rounded-xl border border-slate-200/60 bg-white/90 p-3"><p className="text-lg font-extrabold text-slate-900">{institutionUsers.length}</p><p className="text-[11px] font-semibold text-slate-500">Users</p></div>
                  <div className="rounded-xl border border-slate-200/60 bg-white/90 p-3"><p className="text-lg font-extrabold text-slate-900">{teacherCount}</p><p className="text-[11px] font-semibold text-slate-500">Teachers</p></div>
                  <div className="rounded-xl border border-slate-200/60 bg-white/90 p-3"><p className="text-lg font-extrabold text-slate-900">{studentCount}</p><p className="text-[11px] font-semibold text-slate-500">Students</p></div>
                  <div className="rounded-xl border border-slate-200/60 bg-white/90 p-3"><p className="text-lg font-extrabold text-slate-900">{pendingCount}</p><p className="text-[11px] font-semibold text-slate-500">Pending approvals</p></div>
                  <div className="rounded-xl border border-slate-200/60 bg-white/90 p-3"><p className="text-lg font-extrabold text-slate-900">{activeCoursesCount}</p><p className="text-[11px] font-semibold text-slate-500">Active courses</p></div>
                </div>
              </div>
            </section>
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-1 gap-2 lg:grid-cols-2">
                  {institutionUsers.map((entry) => (
                    <article key={entry.userId} className="rounded-xl border border-slate-200/60 bg-white p-3">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className="rounded-full border border-sky-200 bg-sky-50 px-2 py-0.5 text-[10px] font-semibold text-sky-700">
                          {getRoleLabel(getEffectiveRole(entry))}
                        </span>
                      </div>
                      <div className="mt-2 grid grid-cols-3 gap-2">
                        <div className="rounded-lg border border-slate-200/60 bg-slate-50 p-2 text-center">
